...
static void txcallback(dwDevice_t *dev)
{
  ...
  //thoi gian gui phai cong them 1 luong delayantenna
...
static void rxcallback(dwDevice_t *dev) {
  ...
  // clear rxpacket with lenght 21 byte
  bzero(&rxPacket, MAC802154_HEADER_LENGTH);

  debug("RXCallback(%d): ", dataLength);

  dwGetData(dev, (uint8_t*)&rxPacket, dataLength);
 //Kiem tra xem dia chi Tag nhận có đúng với địa chỉ config hay khong
  ...
  //Copy dia chỉ cua Anchor gui tới cho Tag vào dia chỉ Anchor destination 
  memcpy(txPacket.destAddress, rxPacket.sourceAddress, 8);
  //Copy dia chi cua Anchor gui toi cho Tag vào dia chi của Tag source 
  ...
  switch(rxPacket.payload[TYPE]) {
    ...
    case ANSWER:
      ...
      //thoi gian den phai tru di 1 luong antennadelay
      arival.full -= (ANTENNA_DELAY/2);
      answer_rx = arival;
      break;
    case REPORT:
    {
      //reportpayload de lay timestamp cua anchor gui ve tag de tinh toan 
